...
\* \*\*`main\_analysis.R`\*\*: The core script for simulation, estimation, and economic valuation.
...
The primary data used in this study were obtained from the Global Burden of Disease (GBD) Study 2021 and the World Bank.



To reproduce the analysis, please download the required datasets from the sources listed below and organize them as follows:



1\.  Create a folder named \*\*`data`\*\* in your project directory.

2\.  Download and place the following files into the `data` folder:



\* \*\*`death1.csv`\*\*: TB Mortality Estimates (Source: GBD 2021 Results Tool)

\* \*\*`risk\_deathall.csv`\*\*: Mortality Attributable to Risk Factors (Source: GBD 2021 Results Tool)
...
\* \*\*`gdp\_data.csv`\*\*: GDP per capita (Source: World Bank Open Data / GBD Covariates)

\* \*\*`shapefiles/`\*\*: Subfolder containing world map shapefiles for visualization



\*\*Note\*\*: Ensure the GBD data files include columns for `location\_id`, `age\_id`, `sex\_id`, `cause\_id`, and estimates with upper/lower bounds.
...
\* \*\*Dependencies\*\*: The scripts will automatically install missing packages. Key libraries include `tidyverse`, `readxl`, `triangle`, `yll`, `sf`, and `ggplot2`.
...
1\.  \*\*Setup Data\*\*: Ensure all datasets are placed in the `data` folder as described above.

2\.  \*\*Run Analysis\*\*: Execute `main\_analysis.R` to perform the simulation and generate results in the `results` folder.
